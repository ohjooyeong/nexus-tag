...
import useCreateClassLabel from '../../_hooks/use-update-class-label';
...
const UpdateClassLabelDialog = ({
  ...
  const updateClassLabelFormSchema = (classLabels: any[] = []) =>
    z.object({
      name: z
        .string()
        .min(1, {
        ...
        .refine(
          (name) =>
            name.toLowerCase() === classLabel.name.toLowerCase() ||
            !classLabels?.some(
  ...
  const classLabelFormSchema = updateClassLabelFormSchema(classLabels);
  ...
  const onSubmit = async (data: ClassLabelFormValues) => {
    setIsLoading(true);
    try {
      const response = await updateClassLabelMutation.mutateAsync({
        name: data.name,
...
export default UpdateClassLabelDialog;
